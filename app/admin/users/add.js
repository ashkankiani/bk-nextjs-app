...
import {hookAddUser} from "@/hooks/admin/hookUser";
...
export default function AddUser() {
  ...
  const addUser = async data => {
    setLoading(true)
    await hookAddUser(data, (response, message) => {
      setLoading(false)
      if (response) {
  ...
  return (
    <>
      <HeadPage title="افزودن کاربر جدید"/>
            ...
            <div className="panel-col-100">
              <button
                ...
                {loading ? (
                  ...
                  <span><AiOutlineSave size="24px" className="inline-flex align-middle ml-2"/>ثبت کاربر</span>
                )}
              </button>
            </div>
          </div>
