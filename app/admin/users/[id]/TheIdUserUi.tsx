...
export default function TheIdUserUi() {
  ...
  const {data: dataCatalogs, isLoading: isLoadingCatalogs, isFetched:isFetchedCatalogs } = useGetCatalogs()
  const {data: dataUser, isLoading: isLoadingUser, isFetched: isFetchedUser} = useShowUser(id , {
  ...
  const onSubmit = async (data: TypeApiUpdateUserReq) => {
    ...
    await mutateAsyncUpdateUser(data).then((res) => {
              ...
              <div className="flex-center-start flex-wrap">
                ...
                <div className="panel-col-100">
                  <button
                    className={
                      'panel-form-submit ' +
                      (isPendingUpdateUser ? 'disable-action' : '')
                    }
                    type="submit">
                    {isPendingUpdateUser ? (
                      <TheSpinner/>
