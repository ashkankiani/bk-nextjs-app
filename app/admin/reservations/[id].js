...
export default function EditReservation({ id }) {
  ...
  const handlerSetService = async IdService => {
    setValue(
      'smsToAdminService',
      services.filter(item => item.id === IdService)[0].smsToAdminService
    )
    setValue(
      'smsToAdminProvider',
      services.filter(item => item.id === IdService)[0].smsToAdminProvider
    )
    setValue('smsToUserService', services.filter(item => item.id === IdService)[0].smsToUserService)
    setValue(
      'emailToAdminService',
      services.filter(item => item.id === IdService)[0].emailToAdminService
    )
    setValue(
      'emailToAdminProvider',
      services.filter(item => item.id === IdService)[0].emailToAdminProvider
    )
    setValue(
      'emailToUserService',
      services.filter(item => item.id === IdService)[0].emailToUserService
    )
  }
      ...
      <div className="panel-main">
        <form className="panel-boxed" onSubmit={handleSubmit(onSubmit)}>
          <div className="flex-center-start flex-wrap">
            ...
            <div className="panel-col-25">
              <label onClick={() => setValue('userId', '2')}>
