...
export default function TheDiscountsUi() {
    ...
    const {mutateAsync: mutateAsyncDeleteDiscount, isPending: isPendingDeleteDiscount} = useDeleteDiscount()

    const handlerDeleteDiscount = async (id: number, close: () => void) => {
        await mutateAsyncDeleteDiscount({id}).then((res) => {
                                                        ...
                                                        >
                                                            {(close: () => void) =>
                                                                <div className="panel-wrapper-modal max-w-[500px]">
                                                                    ...
                                                                    />
                                                                    <div className="panel-modal-title">حذف کد تخفیف
                                                                    </div>
                                                                    <div className="panel-modal-content">
                                                                        <p>آیا از حذف کدتخفیف<strong
                                                                            className="px-1 text-red-500">{item.title}</strong>مطمن
                                                                            هستید؟ بدانید که دیگر
                                                                            نمیتوانید بفهمید که، کاربر از چه کد تخفیفی
                                                                            روی فاکتور استفاده کرده است.</p>
                                                                    </div>
                                                                    <div className="panel-modal-footer">
                                                                        <div
                                                                            className={"panel-modal-confirm-delete " + (isPendingDeleteDiscount ? "disable-action" : "cursor-pointer")}
