...
import {hookGetProviderByKeyId} from "@/hooks/admin/hookProvider";
...
import {hookAddReservation, hookIsReservation} from "@/hooks/admin/hookReservation";
...
export default function AddReservation() {
  ...
  const onSubmit = data => {
    ...
    data.trackingCode = generateTrackingCodeWithDate()
  ...
  const handlerGetProvidersWhere = async (id) => {
    setValue("providerId", "")
  ...
  const handlerSetUser = async (IdUser) => {
    if (serviceGender !== "NONE") {
      let gender = users.filter(item => item.id === IdUser)[0].gender
      if (serviceGender !== gender) {
        bkToast('error', "این نوبت مخصوص " + textGenderType(serviceGender) + " میباشد.")
        setValue("userId", "")
      }
    }
  }
  ...
  return (
    <>
      <HeadPage title="افزودن رزرو جدید"/>
      ...
      <div className="panel-main">
        <form className="panel-boxed" onSubmit={handleSubmit(onSubmit)}>
          <div className="flex-center-start flex-wrap">
            ...
            <div className="panel-col-25">
              <label>انتخاب کاربر<span>*</span></label>
              <select
                {...register('userId', {
                  valueAsNumber: true,
                  required: {
                      ...
                      <>
                        <option value="" disabled selected>انتخاب کنید</option>
                        {
                          users?.map((item, index) =>
                            <option key={index} value={item.id}>{item.fullName + " - " + item.mobile}</option>
                          )
                        }
                      </>
                      :
            ...
            <div className="panel-col-100">
              <button
                ...
                {loading ? (
                  ...
                  <span><AiOutlineSave size="24px" className="inline-flex align-middle ml-2"/>ثبت رزرو</span>
                )}
              </button>
            </div>
