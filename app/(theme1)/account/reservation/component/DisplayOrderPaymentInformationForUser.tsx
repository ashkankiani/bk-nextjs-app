...
import {PNtoEN, textDiscountsType, textPaymentType, textSettingsBankName} from "@/libs/utility";
...
export default function DisplayOrderPaymentInformationForUser({item, close}) {

    let dataExport = []

    dataExport.push(['سرویس', item.order.provider.service.name]);
    dataExport.push(['ارائه دهنده', item.order.provider.user.fullName]);
    dataExport.push(['تاریخ رزرو', item.date]);
    dataExport.push(['ساعت رزرو', item.time]);
    dataExport.push(['روز', fullStringToDateObjectP(item.date).weekDay.name]);
    ...
    if (item.order.payment.transaction) {
        ...
        dataExport.push(['نام درگاه', textSettingsBankName(item.order.payment.transaction.bankName)]);
        dataExport.push(['مبلغ واریزی )تومان(', numberWithCommas(item.order.payment.transaction.amount)]);
        ...
        dataExport.push(['تاریخ واریز', PNtoEN(fullStringToDateObjectP(item.order.payment.transaction.createdAt, 'YYYY-MM-DDTHH:MM:SS.SSSZ').format('YYYY/MM/DD - HH:MM'))]);
        if (item.order.discount) {
            dataExport.push(['کد تخفیف', item.order.discount.title + ")" + item.order.discount.code + "("]);
            dataExport.push(['نوع تخفیف', textDiscountsType(item.order.discount.type)]);
            dataExport.push(['میزان تخفیف', item.order.discount.type === 'CONSTANT' ? numberWithCommas(item.order.discount.amount) : item.order.discount.amount + ' ' + item.order.discount.type === 'CONSTANT' ? 'تومان' : 'درصد']);
        ...
        // dataExport.push(['مبلغ درآمد )تومان(', item.order.discountPrice ? numberWithCommas(item.order.price - item.order.discountPrice) : numberWithCommas(item.order.price)]);
    ...
    return (
        <div className="panel-wrapper-modal max-w-[500px]">
            ...
            <div className="panel-modal-content">
                <div className="flex-center-between flex-wrap gap-2 mb-4">
                    <div>روش پرداخت</div>
                    <div>
                ...
                </div>
                {
                    ...
                    <>
                        <div className="flex-center-between flex-wrap gap-2 mb-4">
                            <div>نام درگاه</div>
                            <div>{textSettingsBankName(item.order.payment.transaction.bankName)}</div>
                        </div>
                        <div className="flex-center-between flex-wrap gap-2 mb-4">
                            <div>مبلغ واریزی</div>
...
{/*                        <div className="flex-center-between flex-wrap gap-2 mb-4">
                        ...
                        <div className="flex-center-between flex-wrap gap-2 mb-4">
                            <div>تاریخ واریز</div>
                            <div
                                dir="ltr">{PNtoEN(fullStringToDateObjectP(item.order.payment.transaction.createdAt, 'YYYY-MM-DDTHH:MM:SS.SSSZ').format('YYYY/MM/DD - HH:MM'))}</div>
                        </div>
                    </>
                }
                {
                    ...
                    <>
                        <div className="flex-center-between flex-wrap gap-2 mb-4">
                            <div>کد تخفیف</div>
                            <div>{item.order.discount.title} ({item.order.discount.code})</div>
                        </div>
                        <div className="flex-center-between flex-wrap gap-2 mb-4">
                            <div>نوع تخفیف</div>
                        ...
                        <div className="flex-center-between flex-wrap gap-2 mb-4">
                            <div>میزان تخفیف</div>
                            <div>{item.order.discount.type === 'CONSTANT' ? numberWithCommas(item.order.discount.amount) : item.order.discount.amount} {item.order.discount.type === 'CONSTANT' ? 'تومان' : 'درصد'}</div>
                        </div>
                        <div className="flex-center-between flex-wrap gap-2 mb-4">
...
{/*                <div className="flex-center-between flex-wrap gap-2 mb-4">
            ...
            <div className="panel-modal-footer">
                <div className="panel-modal-confirm flex-center-center gap-2">
                    ...
                    <GeneratePdf type="FACTOR" data={dataExport} orientation="portrait" title="پرینت فاکتور"/>
                </div>
