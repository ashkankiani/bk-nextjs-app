...
export default function ConnectionTest() {
    ...
    const handlerTestEmail = async (e: React.FormEvent) => {
        ...
        await mutateAsyncSendEmail(data).then((res) => {
    ...
    const handlerTestSms = async (e: React.FormEvent) => {
        ...
        await mutateAsyncSendSms(data).then((res) => {
            // خروجی res رو ببینم
            console.log(res)
                ...
                <>
                    ...
                    <div className="panel-col-100">
                        <div className="prose prose-sm text-neutral-800 dark:text-white max-w-full">
                            ...
                            <p>برای اینکه از اتصال سامانه پیامکی و ایمیل خود مطمن شوید میتوانید از این
                                بخش استفاده کنید. توجه
                                فرمایید ثبت <strong>کد پترن پیامک یکبار مصرف</strong> ضروری است.</p>
                        </div>

                        <form onSubmit={handlerTestSms}
                              className="flex-center-start gap-4 flex-wrap mt-4">
                            <input
                            ...
                            <button
                                ...
                                className={
                                    'bk-button-green ' +
                                    (isPendingSendSms ? 'disable-action' : '')
                                }>
                                {isPendingSendSms ? (
                                    <TheSpinner/>
                                ) : (
                        ...
                        <form onSubmit={handlerTestEmail}
                              className="flex-center-start gap-4 flex-wrap mt-4">
                            <input
                            ...
                            <button
                                ...
                                className={
                                    'bk-button-red ' +
                                    (isPendingSendEmail ? 'disable-action' : '')
                                }>
                                {isPendingSendEmail ? (
                                    <TheSpinner/>
                                ) : (
                                    <span><MdEmail size="24px"
                                                   className="inline-flex align-middle ml-2"/>تست ارسال ایمیـل</span>
                                )}
                            </button>
                        </form>
                    </div>
