...
import { bkToast, textPaymentType, textReservationsStatus, textBankName } from '@/libs/utility'
...
import DisplayOrderPaymentInformationForUser from '@/app/(theme1)/account/reservations/component/DisplayOrderPaymentInformationForUser'
    ...
    <div className="bk-box md:w-10/12">
      ...
      <div className="bk-box-wrapper">
        ...
        <p className="bk-box-wrapper-description">
          در این بخش وضعیت کلی رزروهای خود را میتوانید مشاهده کنید.
        </p>
        <div className="bk-table-transparent relative overflow-x-auto">
          <table className="w-full text-center">
            <thead className="fa-sbold-18px bg-green-100 dark:bg-darkNavy3">
              <tr>
            ...
            <tbody className="fa-regular-18px bg-white dark:bg-darkNavy2">
              {loading ? (
                data.length > 0 ? (
                    ...
                    <tr key={index}>
                      ...
                      <td>
                        ...
                        {item.order.payment.transaction ? (
                          <>
                            <div>درگاه {textBankName(item.order.payment.transaction.bankName)}</div>
                          ...
                          <div className="flex-center-center gap-3">
                            {item.status === 'DONE' && (
                              <Popup
                                ...
                                trigger={
                                  <div className="bk-button cursor-pointer bg-green-500 p-2">
                                    فاکتور
                                  </div>
                                }
                                modal
                              ...
                              >
                                {close => (
                                  <DisplayOrderPaymentInformationForUser
                                    item={item}
                                    close={close}
                                  />
                                )}
                            ...
                            {(setting.cancellationReservationUser && item.status === 'REVIEW') ||
                              (setting.cancellationReservationUser &&
                                item.status === 'COMPLETED' && (
                                  <Popup
                                    className="bg-modal"
                                    contentStyle={{ width: '100%' }}
                                    trigger={
                                      <div className="bk-button cursor-pointer bg-yellow-500 p-2">
                                        لغو
                                      </div>
                                    }
                                    modal
                                    ...
                                    {close => (
                                      <div className="panel-wrapper-modal max-w-[500px]">
                                        ...
                                        <div className="panel-modal-title">
                                          لغو رزرو ({item.order.trackingCode})
                                        </div>
                                        <div className="panel-modal-content">
                                          {checkCancellationDeadline(item.dateTimeStartEpoch) ? (
                                              ...
                                              <p>
                                                <span>
                                                  ...
                                                  <strong className="px-1 text-red-500">
                                                    {
                                                      fullStringToDateObjectP(item.date).weekDay
                                                        .name
                                                    }
                                                  </strong>
                                                </span>
                                            ...
                                            <p>
                                              ...
                                              <strong className="px-1 text-red-500">
                                                {minuteIntegerToTime(
                                                  setting.cancellationDeadline,
                                                  'string'
                                                )}
                                              </strong>
                                              دقیقه قبل از شروع رزرو است. پس نمیتوانید این رزرو را
                                              لغو نمایید.
                                            </p>
                                          )}
                                        </div>
                  ...
                  <tr>
                    <td colSpan={showColumnStatus ? 7 : 6}>رزروی برای نمایش وجود ندارد.</td>
                  </tr>
                )
              ) : (
                <tr>
