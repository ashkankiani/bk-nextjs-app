  ...
  const onSubmit = async (data: TypeFormTheSignInUi) => {
    ...
    if (setting.loginOTP) {
      if (code === Number(data.otp)) {
        ...
        await mutateAsyncSignInOtp(transformedData)
          .then(res => {
            handlerSignIn(res)
          })
          .catch(errors => {
      ...
      await mutateAsyncSignIn(transformedData)
        .then(res => {
          handlerSignIn(res)
        })
        .catch(errors => {
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="relative mb-4 columns-1">
                ...
                <div
                  ...
                  className={
                    ...
                    (isPendingSignInOtp || showTime ? 'disable-action' : 'cursor-pointer')
                  }
                >
                  {isPendingSignInOtp ? <TheSpinner /> : showTime ? timer + ' ثانیه' : text}
                </div>
          ...
          <div className="relative mb-4 columns-1">
            ...
            <div
              ...
              className="en-sbold-20px absolute left-4 top-3 cursor-pointer select-none text-neutral-400"
            >
              {captcha}
            </div>
          ...
          <div className="mb-4 columns-1">
            <button
              className={
                ...
                (isPendingSignIn || isPendingSignInOtp ? 'disable-action' : '')
              }
            >
              {isPendingSignIn || isPendingSignInOtp ? <TheSpinner /> : 'ورود به سیستم'}
            </button>
          </div>
        </form>
        <div className="flex-center-center mt-6 divide-x divide-x-reverse divide-black dark:divide-darkNavy3">
          <Link href="/account/reset-password" className="px-2">
            فراموشی کلمه عبور
          </Link>
