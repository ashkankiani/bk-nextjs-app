import {
    hookAqayePardakhtGetAuthority, hookAqayePardakhtGetVerify,
    hookIdPayGetAuthority, hookIdPayGetVerify,
    hookZarinpalGetAuthority,
    hookZarinpalGetVerify, hookZibalGetAuthority, hookZibalGetVerify
                    ...
                    case 'authority':
                        try {
                            return await new Promise((resolve) => {
                                hookZarinpalGetAuthority(params, (response: any, message: any) => {
                                    if (response) {
                                        resolve(Response.json(message, { status: 200 }));
                                    } else {
                                        resolve(Response.json({ error: message }, { status: 201 }));
                                    }
                                });
                            });
                        } catch {
                            return Response.json({ error: 'Failed to Authority Zarinpal' }, { status: 500 });
                        }
                    case 'verify':
                        try {
                            return await new Promise((resolve) => {
            ...
            case 'IDPAY':
                switch (action) {
                    case 'authority':
                        try {
                            return await new Promise((resolve) => {
                                hookIdPayGetAuthority(params, token, (response: any, message: any) => {
                                    if (response) {
                                        resolve(Response.json(message, { status: 200 }));
                                    } else {
                                        resolve(Response.json(message, { status: 201 }));
                                    }
                                });
                            });
                        } catch {
                            return Response.json({ error: 'Failed to Authority IDPAY' }, { status: 500 });
                        }
                    case 'verify':
                        try {
                            return await new Promise((resolve) => {
                                hookIdPayGetVerify(params, (response: any, message: any) => {
                                    if (response) {
                                        ...
                                        resolve(Response.json({ error: 'سفارش پرداخت نشده یا ناموفق بوده است.' }, { status: 201 }));
                                    }
                                });
                            });
                        } catch {
            ...
            case 'AQAYEPARDAKHT':
                switch (action) {
                    case 'authority':
                        try {
                            return await new Promise((resolve) => {
                                hookAqayePardakhtGetAuthority(params, (response: any, message: any) => {
                                    if (response) {
                                        resolve(Response.json(message, { status: 200 }));
                                    } else {
                                        resolve(Response.json(message, { status: 201 }));
                            ...
                            return Response.json({ error: 'Failed to Authority Aqaye Pardakht' }, { status: 500 });
                        }
                    case 'verify':
                        try {
                            return await new Promise((resolve) => {
            ...
            case 'ZIBAL':
                switch (action) {
                    case 'authority':
                        try {
                            return await new Promise((resolve) => {
                                hookZibalGetAuthority(params, (response: any, message: any) => {
                                    if (response) {
                                        resolve(Response.json(message, { status: 200 }));
                                    } else {
                                        let errors: Record<string, string> = {
                                            ...
                                            105:'قیمت باید بزرگتر از 1,000 ریال باشد.',
                                            106:'کالبک نامعتبر می‌باشد. (شروع با http و یا https)',
                            ...
                            return Response.json({ error: 'Failed to Authority Zibal' }, { status: 500 });
                        }
                    case 'verify':
                        try {
                            return await new Promise((resolve) => {
