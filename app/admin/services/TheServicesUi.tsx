...
export default function TheServicesUi() {
    ...
    const {mutateAsync: mutateAsyncDeleteService, isPending: isPendingDeleteService} = useDeleteService()


    const handlerDeleteService = async (id: number, close: () => void) => {
        await mutateAsyncDeleteService({id}).then((res) => {
                                                        ...
                                                        >
                                                            {(close: () => void) =>
                                                                <div className="panel-wrapper-modal max-w-[480px]">
                                                                    ...
                                                                    />
                                                                    <div className="panel-modal-title">دریافت تاییدیه
                                                                    </div>
                                                                    <div className="panel-modal-content">
                                                                        <p className="leading-9 text-justify">شما در حال
                                                                            حذف<strong
                                                                                className="px-1 text-red-500">{item.name}</strong>هستید.
                                                                            بدانید که ابتدا ارائه
                                                                            دهنده های متصل به سرویس را باید حذف کنید.
                                                                        </p>
                                                                    </div>
                                                                    <div className="panel-modal-footer">
                                                                        <div
                                                                            className={"panel-modal-confirm-delete " + (isPendingDeleteService ? "disable-action" : "cursor-pointer")}
